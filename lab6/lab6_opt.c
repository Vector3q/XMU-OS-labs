...
struct PageFrame
{
	...
	int next_occur_index; 				 // the next time page will work
}PageFrame;

int findFarOccurIndexID(struct PageFrame page_frame[])
{
	int Farest_index = 0;
	int Farest_id = 0;
	for(int i=0; i<frame_number; i++)
	{
		...
		if(Farest_index < page_frame[i].next_occur_index)
		{
			Farest_index = page_frame[i].next_occur_index;
			Farest_id = i;
		}
...
void opt(struct PageFrame page_frame[])
{
	
	printf("Welcome to use opt algorithm. \nNow, just input the number of pages to work\n");
	...
	while(reverse_number--)
	{
		...
		for(int i=0; i<frame_number; i++)
		{
			if(page_frame[i].pageid == visiting_pageid[page_visiting_number - reverse_number - 1])
			{
				...
				hit_flag = 1;
				page_frame[i].next_occur_index = MAXM;
				for(int j = page_visiting_number - reverse_number; j<page_visiting_number; j++)
				{
					if(visiting_pageid[j] == page_frame[i].pageid)
					{
						page_frame[i].next_occur_index = j+1;
						break;
					}
				}
			}	
		...
		//PF fault
		int replace_id = findFarOccurIndexID(page_frame);
		page_frame[replace_id].pageid = visiting_pageid[page_visiting_number - reverse_number - 1];
		page_frame[replace_id].next_occur_index = 0;
		printf("Page frame:%d has repalaced, new working page id:%d\n",replace_id,page_frame[replace_id].pageid);
		
		for(int i = page_visiting_number - reverse_number; i<page_visiting_number; i++)
		{
			if(visiting_pageid[i] == page_frame[replace_id].pageid)
			{
				page_frame[replace_id].next_occur_index = i+1;
				break;
				//printf("The next occur index of id:%d\n",page_frame[replace_id].next_occur_index);
			}
		}
		
		if(page_frame[replace_id].next_occur_index == 0)
		{
			page_frame[replace_id].next_occur_index = MAXM;
		}
	}
